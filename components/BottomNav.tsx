...
const BottomNav: React.FC<BottomNavProps> = ({ currentTab, onTabChange }) => {
  ...
  const getItemClass = (tabName: TabId) => {
    ...
    return `flex flex-col items-center justify-center w-full h-full py-2 cursor-pointer transition-colors duration-200 ${isActive ? 'text-primary' : 'text-gray-400 hover:text-gray-500'
      }`;
  };
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-gray-200 pb-safe-bottom z-50 h-[80px]">
      <div className="flex justify-around items-center h-full w-full max-w-2xl mx-auto px-4">
        ...
        <button className={getItemClass('weekly')} onClick={() => onTabChange('weekly')}>
          <span className={getIconClass(currentTab === 'weekly')}>analytics</span>
          <span className="text-[10px] font-bold">周分析</span>
        </button>
      </div>
    </div>
  );
